...
- Search and extraction: You.com, Serper, Firecrawl, Tavily, Exa, Linkup, Parallel, Keenable, Search1API, and the optional local DonSeTch MCP sidecar. Serper extraction uses its webpage scraper (`scrape.serper.dev`) and sits last in the default auto-extraction fallback chain. Search1API and DonSeTch default to `auto_allow=false` for both capabilities and are explicit-only unless deliberately enabled.
...
- `provider="auto"` will not select SerpBase, Querit, Octen, Search1API, TinyFish, or DonSeTch unless opted in.
