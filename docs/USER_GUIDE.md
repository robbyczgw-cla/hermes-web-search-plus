...
Parallel extraction explicitly requests `full_content`. Its default budget is 60,000 characters per result, and the batch total scales with the number of URLs (`len(urls)` × per-result budget), so long documents and multi-URL batches are not unfairly shortened compared with other extraction providers. Operators can pin explicit request-side limits in `config.json` with `parallel.max_chars_per_result` and `parallel.max_chars_total`. The applied caps are always reported in `metadata.content_caps`, and any result whose content hits the per-result budget is flagged `content_capped: true`.
